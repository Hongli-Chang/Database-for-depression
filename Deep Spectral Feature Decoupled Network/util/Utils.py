...
def PrintScore(true, pred, savePath=None, average='macro'):
    ...
    print("Main scores:")
    print('Acc\tF1\tKappa\tF1_Negative\tF1_Neutral\tF1_Positive',file=saveFile)
    print('%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f' % (metrics.accuracy_score(true, pred),
                                                  ...
                                                  F1[0], F1[1], F1[2]),file=saveFile)
          # Classification report
    print('\nClassification report:', file=saveFile)
    print(metrics.classification_report(true, pred, target_names=['Normal', 'Anxiety', 'depression']), file=saveFile)
    # Confusion matrix
...
def PrintScore1(true, pred, savePath=None, average='macro'):
    if savePath == None:
        saveFile = None
    else:
        saveFile = open(savePath + "Result.txt", 'a+')
    ...
    F1 = metrics.f1_score(true, pred, average=None)
    print("Main scores:")
    print('Acc\tF1\tKappa\tF1_0\tF1_1\tF1_2',file=saveFile)
    print('%.4f\t%.4f\t%.4f\t%.4f\t%.4f' % (metrics.accuracy_score(true, pred),
                                                  metrics.f1_score(true, pred, average=average),
                                                  metrics.cohen_kappa_score(true, pred),
                                                  F1[0], F1[1]),file=saveFile)
          # Classification report
    print('\nClassification report:', file=saveFile)
    print(metrics.classification_report(true, pred, target_names=['Normal', 'Depression']), file=saveFile)
    # Confusion matrix
    print('Confusion matrix:', file=saveFile)
    print(metrics.confusion_matrix(true, pred), file=saveFile)
    # Overall scores
...
def ConfusionMatrix(y_true, y_pred, classes, savePath, title=None, cmap=plt.cm.Blues):
    ...
    plt.show()
    return ax
def ConfusionMatrix1(y_true, y_pred, classes, savePath, title=None, cmap=plt.cm.Blues):
    if not title:
        title = 'Confusion matrix'
    # Compute confusion matrix
    cm = metrics.confusion_matrix(y_true, y_pred)
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, format(cm[i, j] * 100, '.2f') + '%\n' + format(cm_n[i, j], 'd'),
                    ...
                    color="white" if cm[i, j] > thresh else "black")
    fig.tight_layout()
    plt.savefig(savePath + title + ".tif")
    plt.show()
    return ax
