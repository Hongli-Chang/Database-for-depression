...
def evaluate_result_valid(label_test, prob, i, str_input):
    ...
    print('%s: step %d, auc %g, FAR %g, FRR %g, acc %g, acc_avg %g, n_eval %d' % (
    str_input, i, np.mean(auc_all), np.mean(FAR_ALL), np.mean(FRR_ALL), acc, np.mean(ACC_ALL), n_user_train))

    return dic_th, np.mean(auc_all),acc
...
def evaluate_result_valid_fixFAR_pool(test_Y, prob, i, str_input, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_unpack,
                           [(i_user, (test_Y == i_user).astype(int), prob[:, i_user]) for i_user in train_labels])
    ...
    print('%s: step %d, FAR %g, FRR %g, acc %g, acc_avg %g, n_eval %d' % (
    str_input, i, np.mean(FAR_ALL), np.mean(FRR_ALL), acc, np.mean(ACC_ALL), n_user_train))
...
def evaluate_result_valid_fixFAR_nopool(test_Y, prob, i, str_input, num_core=20):
    ...
    for i_user in train_labels:
        ...
        FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user] = sub_fun(i_user,
                                                                                    (test_Y == i_user).astype(int),
                                                                                    prob[:, i_user])
...
def evaluate_result_test_attack_pool(prob, i, str_input, dic_th, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_test_unpack,
                           [(i_user, np.zeros(n), prob[:, i_user], dic_th[i_user]) for i_user in range(n_user_train)])
...
def eval_distBased_valid_fixFAR_pool(train_X, train_Y, test_X, test_Y, i, str_input, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_dist_unpack,
                           [(i_user, test_X, (test_Y == i_user).astype(int), train_X[train_Y == i_user, :]) for i_user
                            in final_labels])
    ...
    print('%s: step %d, FAR %g, FRR %g, acc_avg %g, n_eval %d' % (
    str_input, i, np.mean(FAR_ALL), np.mean(FRR_ALL), np.mean(ACC_ALL), n_user_train))
    #     print('%s: step %d, [%g, %g, %g, %g, %g]' % (str_input, i,  np.mean(auc_all), acc,np.mean(ACC_ALL),np.mean(FAR_ALL),np.mean(FRR_ALL)))

    return dic_th, 1 - np.mean(FRR_ALL)


def sub_fun_dist_testattack(i_user, test_X, Y_test, test_X_attack, Y_test_attack, train_X,
                            argmin_fun=argmin_mean_FRR_st_FAR, exp_frr=0.1):
    dist = mat_dist(test_X, train_X)
    prob = - np.min(dist, axis=1)
...
def eval_distBased_testattack(train_X, train_Y, test_X, test_Y, test_X_attack, test_Y_attack, i, str_input, num_core=20,
                              argmin_fun=argmin_mean_FRR_st_FAR, exp_frr=0.1):
    ...
    start_time = time_simple.clock()
    for i_user in range(n_user_train):
        FAR_ALL[i_user], FRR_ALL[i_user], FAR_ALL_attack[i_user], dic_th[i_user], sum_1_label_test[i_user], \
        sum_1_label_out[i_user], sum_1_label_out_attack[i_user], sum_1_label_test_attack[
            i_user] = sub_fun_dist_testattack(i_user, test_X, test_Y[:, i_user], test_X_attack,
                                              test_Y_attack[:, i_user], train_X[train_Y[:, i_user] == 1, :],
                                              argmin_fun=argmin_fun, exp_frr=exp_frr)

    print('%s: step %d, FAR %g, FRR %g, FAR_attack %g, n_eval_far %d, n_eval_frr %d, n_eval_far_attack %d' % (
    str_input, i, np.mean(FAR_ALL[np.invert(np.isnan(FAR_ALL))]), np.mean(FRR_ALL[np.invert(np.isnan(FRR_ALL))]),
    np.mean(FAR_ALL_attack[np.invert(np.isnan(FAR_ALL_attack))]), np.sum(np.invert(np.isnan(FAR_ALL))),
    np.sum(np.invert(np.isnan(FRR_ALL))), np.sum(np.invert(np.isnan(FAR_ALL_attack)))))

    print('macro:\trecall %g,\t\t\tattack %g' % (
    1. - np.mean(FRR_ALL[np.invert(np.isnan(FRR_ALL))]), np.mean(FAR_ALL_attack[np.invert(np.isnan(FAR_ALL_attack))])))
    print('micro:\trecall %g(%d/%d),\tattack %g(%d/%d)' % (
    np.sum(sum_1_label_out) / np.sum(sum_1_label_test), np.sum(sum_1_label_out), np.sum(sum_1_label_test),
    np.sum(sum_1_label_out_attack) / np.sum(sum_1_label_test_attack),
    np.sum(sum_1_label_out_attack), np.sum(sum_1_label_test_attack)))


def eval_distBased_valid_fixFAR_nopool(train_X, train_Y, test_X, test_Y, i, str_input, num_core=20,
                                       argmin_fun=argmin_mean_FRR_st_FAR):
    if np.ndim(train_Y) == 2 and train_Y.shape[1] > 1:
        ...
        for i_user in range(n_user_train):
            ...
            FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user], sum_1_label_test[i_user], \
            sum_1_label_out[i_user] = sub_fun_dist(i_user, test_X, test_Y[:, i_user],
                                                   train_X[train_Y[:, i_user] == 1, :], argmin_fun=argmin_fun)
        ...
        for i_user in final_labels:
            ...
            FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user], sum_1_label_test[i_user], \
            sum_1_label_out[i_user] = sub_fun_dist(i_user, test_X, (test_Y == i_user).astype(int),
                                                   train_X[train_Y == i_user, :], argmin_fun=argmin_fun)
...
def eval_distBased_valid_fixFAR_nopool_negDownsample(train_X, train_Y, test_X, test_Y, i, str_input, num_core=20,
                                                     n_neg=10000, flag_print=True):
    test_labels = np.unique(test_Y)
    train_labels = np.unique(train_Y)
    final_labels = np.intersect1d(train_labels, test_labels)
    ...
    for i_user, e_user in enumerate(final_labels):
        ...
        FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user] = sub_fun_dist(i_user, test_X_proc,
                                                                                         test_Y_proc,
                                                                                         train_X[train_Y == e_user, :])
...
def eval_angleBased_valid_fixFAR_nopool_negDownsample_sameDeviceType(train_X, train_Y, test_X, test_Y, i, str_input,
                                                                     num_core=20, n_neg=10000, flag_print=True):
    ...
    for i_user, e_user in enumerate(final_labels):
        ...
        FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user] = sub_fun_angle(i_user, test_X_proc,
                                                                                          test_Y_proc, train_X[
                                                                                                       train_Y[:,
                                                                                                       0] == e_user, :])
...
def eval_distBased_valid_fixFAR_nopool_negDownsample_sameDeviceType(train_X, train_Y, test_X, test_Y, i, str_input,
                                                                    num_core=20, n_neg=10000, flag_print=True):
    test_labels = np.unique(test_Y[:, 0])
    train_labels = np.unique(train_Y[:, 0])
    final_labels = np.intersect1d(train_labels, test_labels)
...
def read_data_metric_pool(file_list, flag_zscore=False, num_core=20, flag_delete_anomaly=False, flag_multilabel=False,
...
def read_h5andcsv(file_path_csv, file_path_h5, flag_delete_anomaly=False, flag_multilabel=False, flag_typecoded=True,
                  flag_includeTime=False, flag_191feat=False, flag_select_device_type=False, selected_device_type=None,
                  flag_no_pressure=False):
...
def read_data_metric_pool_h5andcsv(file_list, flag_zscore=False, num_core=20, flag_delete_anomaly=False,
                                   flag_multilabel=False, mean_x=None, std_x=None, flag_typecoded=True,
                                   flag_includeTime=False, flag_191feat=False, flag_select_device_type=False,
                                   selected_device_type=None, flag_float32=False, flag_no_pressure=False):
    #     print file_list
    len_file_list = len(file_list)
    n_core = int(np.minimum(len_file_list, num_core))
    if n_core > 1:
        with poolcontext(processes=n_core) as pool:
            results = pool.map(read_h5andcsv_unpack, [(file_path[0], file_path[1], flag_delete_anomaly, flag_multilabel,
                                                       flag_typecoded, flag_includeTime, flag_191feat,
                                                       flag_select_device_type, selected_device_type, flag_no_pressure)
                                                      for file_path in file_list])
        X, Y = np.vstack([e[0] for e in results]), np.vstack([e[1] for e in results])
    else:
        X, Y = read_h5andcsv(file_list[0][0], file_list[0][1], flag_delete_anomaly, flag_multilabel, flag_typecoded,
                             flag_includeTime, flag_191feat, flag_select_device_type, selected_device_type,
                             flag_no_pressure)
...
def read_data_metric_pool_h5andcsv_v1(file_list, flag_zscore=False, num_core=20, flag_delete_anomaly=False,
                                      flag_multilabel=False, mean_x=None, std_x=None, flag_typecoded=True,
                                      flag_includeTime=False, flag_191feat=False, flag_select_device_type=False,
...
def eval_distBased_trainValid_fixFAR_negDownsample(i, str_input, test_list, batch_size_list_test,
                                                   Model_gen_hidden_feature_branch, flag_multilabel=False,
                                                   feature_dim=128, read_method=read_data_metric_pool, flag_zscore=True,
                                                   nsample=40, mean_x=None, std_x=None, flag_multimodel=False,
                                                   dic_deviceType2ID=None, flag_typecoded=True, flag_191feat=False,
                                                   flag_select_device_type=False, selected_device_type=None,
                                                   flag_test_same_device=True, flag_delete_anomaly=False,
                                                   flag_float32=False, flat_time_float32=False, flag_gen_mask=False,
                                                   session=None, mask=None, inputs=None, input_target=None,
                                                   batch_size=512, reader_vector_class_sort=None, ds='ds20190414',
                                                   flag_no_pressure=False):
    cur_list_test = 0
    i_prob = 0
    FAR_ALL = []
    ...
    while (cur_list_test + 1) * batch_size_list_test <= len(test_list):
        ...
        test_train_X, test_train_y_a = read_method(test_list_tmp, flag_zscore=flag_zscore,
                                                   flag_delete_anomaly=flag_delete_anomaly, mean_x=mean_x, std_x=std_x,
                                                   flag_multilabel=flag_multilabel, flag_typecoded=flag_typecoded,
                                                   flag_191feat=flag_191feat, flag_includeTime=True,
                                                   flag_select_device_type=flag_select_device_type,
                                                   selected_device_type=selected_device_type, flag_float32=False,
                                                   flag_no_pressure=flag_no_pressure)

        timestamp = test_train_X[:, 1]
        if flat_time_float32:
            timestamp = timestamp.astype(np.float32)
        test_train_idx, test_test_idx, test_num_sample = split_train_test_fix_nsample_timeordered(
            test_train_y_a[:, i_prob], timestamp, test_rate=0.2, nsample=nsample)
        ...
        if flag_gen_mask:
            ...
            data_reader_test_train = reader_vector_class_sort.Reader(test_train_X, test_train_y_a,
                                                                     batch_size=batch_size, flag_shuffle=False)
            data_reader_test_train_rand = reader_vector_class_sort.Reader(test_train_X, test_train_y_a,
                                                                          batch_size=batch_size, flag_shuffle=True)
        ...
        if flag_multimodel:
            ...
            train_X_f = Model_gen_hidden_feature_branch.predict(
                [test_train_X, to_categorical(test_train_y_a[:, 1].astype(int), len(dic_deviceType2ID))])
            test_test_y_a[:, 1] = label2uniqueID_sub_test(test_test_y_a[:, 1], dic_deviceType2ID)[:, 0]
            test_X_f = Model_gen_hidden_feature_branch.predict(
                [test_test_X, to_categorical(test_test_y_a[:, 1].astype(int), len(dic_deviceType2ID))])
        else:
        ...
        if flag_multilabel:
            ...
            if flag_test_same_device:
                _, _, far, frr = eval_distBased_valid_fixFAR_nopool_negDownsample_sameDeviceType(train_X_f,
                                                                                                 test_train_y_a,
                                                                                                 test_X_f,
                                                                                                 test_test_y_a, 0, '',
                                                                                                 flag_print=False)
            else:
                _, _, far, frr = eval_distBased_valid_fixFAR_nopool_negDownsample(train_X_f, test_train_y_a[:, i_prob],
                                                                                  test_X_f, test_test_y_a[:, i_prob], 0,
                                                                                  '', flag_print=False)
        ...
        if cur_list_test == 1:
            ...
            mean_far, mean_frr = (mean_far * cont_far + np.sum(far)) / float(sum_cont_far), (
                        mean_frr * cont_frr + np.sum(frr)) / float(sum_cont_frr)
    ...
    print('%s: step %d, FAR %g, FRR %g, n_eval_far %d, n_eval_frr %d' % (
    str_input, i, mean_far, mean_frr, cont_far, cont_frr))
    return 1 - mean_frr


def eval_angleBased_trainValid_fixFAR_negDownsample(i, str_input, test_list, batch_size_list_test,
                                                    Model_gen_hidden_feature_branch, flag_multilabel=False,
                                                    feature_dim=128, read_method=read_data_metric_pool,
                                                    flag_zscore=True, nsample=40, mean_x=None, std_x=None,
                                                    flag_multimodel=False, dic_deviceType2ID=None, flag_typecoded=True,
                                                    flag_191feat=False, flag_select_device_type=False,
                                                    selected_device_type=None, flag_test_same_device=True):
    cur_list_test = 0
    i_prob = 0
    FAR_ALL = []
    FRR_ALL = []
    ...
    while (cur_list_test + 1) * batch_size_list_test <= len(test_list):
        test_list_tmp = test_list[cur_list_test * batch_size_list_test:(cur_list_test + 1) * batch_size_list_test]
        cur_list_test = cur_list_test + 1
        test_train_X, test_train_y_a = read_method(test_list_tmp, flag_zscore=flag_zscore, flag_delete_anomaly=False,
                                                   mean_x=mean_x, std_x=std_x, flag_multilabel=flag_multilabel,
                                                   flag_typecoded=flag_typecoded, flag_191feat=flag_191feat,
                                                   flag_includeTime=True,
                                                   flag_select_device_type=flag_select_device_type,
        ...
        if flag_multilabel:
            ...
            if flag_test_same_device:
                _, _, far, frr = eval_angleBased_valid_fixFAR_nopool_negDownsample_sameDeviceType(train_X_f,
                                                                                                  test_train_y_a,
                                                                                                  test_X_f,
                                                                                                  test_test_y_a, 0, '',
                                                                                                  flag_print=False)
...
def eval_offline(read_path, use_wechet_data=True, extract_feature=True, use_default_model=False,
    ...
    num_core = 20
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_offline_unpack, [
            (i, clf, train_X, label_train[:, i], test_X, label_test[:, i], test_X_attack, label_test_attack[:, i]) for i
            in range(label_train.shape[1])])
    ...
    print('FAR %g, FRR %g, acc %g, FAR_ATTACK %g' % (
    np.mean(FAR_ALL), np.mean(FRR_ALL[np.invert(np.isnan(FRR_ALL))]), np.mean(ACC_ALL),
    np.mean(FAR_ALL_attack[np.invert(np.isnan(FAR_ALL_attack))])))
    ...
    eval_distBased_testattack(train_X, label_train,
                              test_X, label_test, test_X_attack, label_test_attack, 0, '', argmin_fun=argmin_fixFRR,
                              exp_frr=0.2)
...
def eval_distBased_test_attack_pool(train_X, train_Y, test_X, i, str_input, dic_th, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_dist_test_unpack,
                           [(i_user, test_X, np.zeros(n), train_X[train_Y == i_user, :], dic_th[i_user]) for i_user in
                            final_labels])
...
def eval_distBased_test_attack_nopool_real_attack(train_X, train_Y, test_X, test_Y, i, str_input, dic_th, num_core=20):
    ...
    for i_order, i_user in enumerate(final_labels):
        ...
        FRR_ALL[i_order] = sub_fun_dist_test_real_attack(i_user, test_X, (test_Y == i_user).astype(int),
                                                         train_X[train_Y == i_user, :], dic_th[i_user])
...
def eval_angleBased_valid_fixFAR_pool(train_X, train_Y, test_X, test_Y, i, str_input, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_angle_unpack,
                           [(i_user, test_X, (test_Y == i_user).astype(int), train_X[train_Y == i_user, :]) for i_user
                            in final_labels])
...
def eval_angleBased_valid_fixFAR_nopool(train_X, train_Y, test_X, test_Y, i, str_input, num_core=20):
    ...
    for i_user in final_labels:
        ...
        FAR_ALL[i_user], FRR_ALL[i_user], ACC_ALL[i_user], dic_th[i_user] = sub_fun_angle(i_user, test_X,
                                                                                          (test_Y == i_user).astype(
                                                                                              int),
                                                                                          train_X[train_Y == i_user, :])
...
def eval_angleBased_test_attack_pool(train_X, train_Y, test_X, i, str_input, dic_th, num_core=20):
    ...
    with poolcontext(processes=num_core) as pool:
        results = pool.map(sub_fun_angle_test_unpack,
                           [(i_user, test_X, np.zeros(n), train_X[train_Y == i_user, :], dic_th[i_user]) for i_user in
                            final_labels])
