...
def input_model_simpleCNN(hidden_dim,input_shape): 
    ...
    drop = Dropout(0.25, name="drop")(conv2)
...
def input_model_shallowCNN(hidden_dim, input_shape):
    inputs = Input(input_shape, name="input")
    conv1 = Conv2D(5, (1, 3), input_shape=input_shape, kernel_constraint=max_norm(2., axis=(0, 1, 2)))(inputs)
    conv2 = Conv2D(5, (62, 1), use_bias=False, kernel_constraint=max_norm(2., axis=(0, 1, 2)))(conv1)
    block1 = BatchNormalization(axis=-1, epsilon=1e-05, momentum=0.1)(conv2)
    block1 = Activation(square)(block1)

    # block1 = AveragePooling2D(pool_size=(1, 2), strides=(1, 2))(block1)
    # block1 = Activation(log)(block1)
    drop = Dropout(0.25)(block1)
    flat = Flatten()(drop)
...
def input_model_simpleCNN1(hidden_dim,input_shape):
    inputs = Input(input_shape, name="input")
    conv1 = Conv2D(32, (3, 3),  activation='relu', name="conv1")(inputs)
    conv2 = Conv2D(32, (3, 3), activation='relu', name="conv2")(conv1)
    pool = MaxPooling2D((2, 1), name="pool1")(conv2)
    drop = Dropout(0.25, name="drop")(pool)
    flat = Flatten(name="flat")(drop)
    model_input = Model(inputs, flat)
...
def input_model_simpleCNN_convout(hidden_dim,input_shape): 
    inputs = Input(input_shape, name="input")
    conv1 = Conv2D(hidden_dim, 3, 3, activation='relu', name="conv1")(inputs)
    #conv2 = Convolution2D(hidden_dim, 3, 3, activation='relu', name="conv2")(conv1)
    pool = MaxPooling2D((2, 2), name="pool1")(conv1)
    drop = Dropout(0.25, name="drop")(pool)
    flat = Flatten(name="flat")(drop)
    model_input = Model(inputs, flat)
...
def input_model_simpleCNNv2(hidden_dim,input_shape):
    inputs = Input(input_shape, name="input")
    conv1 = Conv2D(hidden_dim, (3, 3), activation='relu', name='conv1', padding='same')(inputs)
    conv2 = Conv2D(hidden_dim, (3, 3), activation='relu', name='conv2', padding='same')(conv1)
    pool = MaxPooling2D((2, 2), name="pool1")(conv2)
    drop = Dropout(0.25, name="drop")(pool)
    flat = Flatten(name="flat")(drop)
    model_input = Model(inputs, flat)
...
def input_model_lstm_deep(hidden_dim,input_shape,depth,inputs=None,input_layer=None,flag_addBN = False,flag_include_zscore=False,zscore_weights=None,flag_outer_swish=False,flag_nodropout=False, nb_classes=2):
    #pdb.set_trace()
    if inputs == None:
        inputs = Input(input_shape, name="input") 
        if flag_include_zscore:
...
def input_LSTM_Attention(hidden_dim,input_shape):
    inputs = Input(input_shape, name="input")
    x0 = Bidirectional(LSTM(hidden_dim, return_sequences=True))(inputs)
    #x1 = Attention()(x0)
    x2 = swish(x0)
    flat= Dense(hidden_dim, activation='linear')(x2)
    model = Model(inputs, flat)
    opt = Adam(lr=1e-3,beta_1=0.5,beta_2=0.999) #SGD(lr=lr)
