...
def gen_raw_data(M):
    train_X, train_y_a, test_X, test_y_a = M.train_data, M.train_label, M.test_data, M.test_label
    #pdb.set_trace()
    train_X = train_X.transpose(2,1,0)
    test_X = test_X.transpose(2,1,0)

    train_y_a = train_y_a.transpose(1,0)
    test_y_a = test_y_a.transpose(1,0)
    ...
    #test_X = test_X.astype(np.float32) / 255n
