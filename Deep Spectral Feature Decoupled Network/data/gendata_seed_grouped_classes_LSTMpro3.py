...
def gen_raw_data(path):
    ...
    f = h5py.File(path)
    train_X, train_y_a, test_X, test_y_a, phase4_X = f["train_X"], f["train_y_a"], f["test_X"], f["test_y_a"], f["phase4_X"]

    train_X = train_X[()].reshape(train_X.shape[0], 1, 5)
    test_X = test_X[()].reshape(test_X.shape[0], 1, 5)
    phase4_X = phase4_X[()].reshape(phase4_X.shape[0], 1, 5)
    ...
    return train_X, train_y_a, test_X, test_y_a, train_y_c, test_y_c, ATTR_NUM, CLASS_NUM, input_shape, lambda_mat, prior_list, phase4_X